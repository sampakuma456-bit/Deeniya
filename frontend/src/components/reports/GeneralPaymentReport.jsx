...
export default function GeneralPaymentReport() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to fetch general payments');
    ...
    <div className="report-print space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3 bg-slate-50 dark:bg-slate-900/30 rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
        ...
        <div className="text-sm text-slate-600 dark:text-slate-400">
          <span className="font-semibold text-slate-900 dark:text-white">General Payment Transaction</span>
      ...
      <ReportExportButtons tableId="general-payment-table" filename={`General_Payment_${dateLabel.replace(/\s/g, '_')}`} />

      <div className="overflow-x-auto">
        <table id="general-payment-table" className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-sm">
              <th className="py-3 px-4 font-semibold">#</th>
              <th className="py-3 px-4 font-semibold">Date</th>
              <th className="py-3 px-4 font-semibold">Payment No</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
            {reportData.length === 0 ? (
              <tr>
                <td colSpan="8" className="py-8 text-center text-slate-400">No payments found.</td>
              </tr>
            ) : (
              reportData.map((r, i) => (
                <tr key={r._id || i} className="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                  ...
                  <td className="py-3 px-4 font-medium text-slate-900 dark:text-white">{r.paymentNumber}</td>
          ...
          {reportData.length > 0 && (
            <tfoot>
              <tr className="bg-slate-100 dark:bg-slate-900 border-t-2 border-slate-300 dark:border-slate-700 font-bold text-sm text-slate-900 dark:text-white">
                <td colSpan="6" className="py-3 px-4 uppercase tracking-wide text-slate-500 dark:text-slate-400 text-xs">Total ({reportData.length} payments)</td>
